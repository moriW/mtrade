...
def test_universe_filter(sample_data):
    ...
    symbols = uf.filter(
        date(2023, 1, 5), min_list_days=30, exclude_st=True, exclude_suspended=True
    )
    assert len(symbols) == 0

    symbols_no_susp = uf.filter(
        date(2023, 1, 4), min_list_days=30, exclude_st=True, exclude_suspended=True
    )
...
def test_pipeline_processing():
    idx = pd.MultiIndex.from_product(
        [
            ...
            pd.date_range("2023-01-01", "2023-01-02"),
        ],
