...
def test_portfolio_builder():
    ...
    weights = PortfolioBuilder.generate_target_weights(scores, top_n=2, weight_method="equal")
...
def test_cost_and_slippage():
    cost_model = CostModel(commission_rate=0.001, min_commission=5.0, stamp_tax_rate=0.001, transfer_fee_rate=0.0)
...
def test_daily_engine_execution():
    dates = [pd.to_datetime("2023-01-01"), pd.to_datetime("2023-01-02"), pd.to_datetime("2023-01-03")]
    bars = pd.DataFrame([
    ...
    idx = pd.MultiIndex.from_product([[dates[0]], ["A"]], names=["trade_date", "symbol"])
