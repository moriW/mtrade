...
class AkShareProvider(DataProvider):
    ...
    def get_daily_bars(self, symbol: str, start_date: date, end_date: date) -> pd.DataFrame:
        try:
            ...
            cols = ["trade_date", "symbol", "open", "high", "low", "close", "volume", "amount"]
    ...
    def get_adjust_factors(self, symbol: str, start_date: date, end_date: date) -> pd.DataFrame:
        try:
            code = symbol.split('.')[0]
            df_unadj = ak.stock_zh_a_hist(symbol=code, period="daily", start_date=start_date.strftime("%Y%m%d"), end_date=end_date.strftime("%Y%m%d"), adjust="")
            df_qfq = ak.stock_zh_a_hist(symbol=code, period="daily", start_date=start_date.strftime("%Y%m%d"), end_date=end_date.strftime("%Y%m%d"), adjust="qfq")
    ...
    def get_limit_prices(self, symbol: str, start_date: date, end_date: date) -> pd.DataFrame:
        ...
        return df[["trade_date", "symbol", "limit_up", "limit_down", "source", "as_of_date", "ingested_at", "version"]]

    def get_suspension_info(self, symbol: str, start_date: date, end_date: date) -> pd.DataFrame:
        df = self.get_daily_bars(symbol, start_date, end_date)
