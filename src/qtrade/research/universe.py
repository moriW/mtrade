...
class UniverseFilter:
    ...
    def filter(self, 
        ...
        valid_master = self.master[
            ...
            (self.master["delist_date"].isna() | (pd.to_datetime(self.master["delist_date"]) > target_ts))
        ]
        
        if min_list_days > 0:
            valid_master = valid_master[
                (target_ts - pd.to_datetime(valid_master["list_date"])).dt.days >= min_list_days
            ]
        ...
        if exclude_suspended and not self.suspensions.empty:
            ...
            susp = self.suspensions[pd.to_datetime(self.suspensions["trade_date"]) == target_ts_dt]
            if not susp.empty:
        ...
        if min_volume is not None and min_volume > 0 and not self.bars.empty:
            target_ts_dt = pd.to_datetime(target_date)
            day_bars = self.bars[pd.to_datetime(self.bars["trade_date"]) == target_ts_dt]
            if not day_bars.empty:
                liquid_symbols = day_bars[day_bars["volume"] >= min_volume]["symbol"].tolist()
