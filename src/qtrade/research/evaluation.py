...
class FactorEvaluator:
    ...
    @staticmethod
    def calc_ic(
        factor_panel: pd.Series, fwd_returns: pd.Series, method: str = "pearson"
    ) -> pd.DataFrame:
        df = pd.concat(
            [factor_panel.rename("factor"), fwd_returns.rename("fwd_ret")], axis=1
        ).dropna()
        if df.empty:
            return pd.DataFrame()
    ...
    def calc_quantile_returns(
        quantiles_panel: pd.Series, fwd_returns: pd.Series
    ) -> pd.DataFrame:
        """
        计算分层收益
        """
        df = pd.concat(
            [quantiles_panel.rename("quantile"), fwd_returns.rename("fwd_ret")], axis=1
        ).dropna()
        if df.empty:
            return pd.DataFrame()

        q_ret = (
            df.groupby(["trade_date", "quantile"])["fwd_ret"].mean().unstack("quantile")
        )
        return q_ret

    @classmethod
    def generate_report(
        cls, factor_panel: pd.Series, bars: pd.DataFrame, horizon: int = 1
    ) -> Dict[str, Any]:
        ...
        report = {
            ...
            if not ic_df.empty and ic_df["ic"].std() != 0
            else 0.0,
            "rank_ic_mean": rank_ic_df["ic"].mean() if not rank_ic_df.empty else 0.0,
            "rank_ic_ir": (rank_ic_df["ic"].mean() / rank_ic_df["ic"].std())
            if not rank_ic_df.empty and rank_ic_df["ic"].std() != 0
            else 0.0,
            "coverage": len(common_idx) / len(factor_panel)
            if len(factor_panel) > 0
            else 0.0,
            "quantile_returns_mean": q_returns.mean().to_dict()
