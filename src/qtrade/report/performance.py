...
class PerformanceAnalyzer:
    def __init__(
        self, history_equity: list, history_trades: list, risk_free_rate: float = 0.02
    ):
        ...
        if not self.equity_df.empty:
            ...
            self.equity_df["returns"] = (
                self.equity_df["equity"].pct_change().fillna(0.0)
            )
    ...
    def compute_metrics(self) -> dict:
        ...
        total_return = (
            self.equity_df["equity"].iloc[-1] / self.equity_df["equity"].iloc[0]
        ) - 1
