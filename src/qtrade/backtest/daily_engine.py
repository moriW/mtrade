...
class DailyBacktestEngine:
    ...
    def run(self):
        ...
        rebalance_dict = {}
        dates_with_signals = self.target_weights.index.get_level_values("trade_date").unique().sort_values()
    ...
    def _rebalance(self, t_date: date, target_w: pd.Series, day_bars: pd.DataFrame, current_value: float):
        if day_bars.empty:
            return
        ...
        for sym, vol, p in buy_orders:
            ...
            if self.cash >= required_cash:
                ...
                if new_vol >= 100:
                    new_cost = self.cost_model.calculate(new_vol, exec_p, OrderDirection.LONG)
                    new_req = (new_vol * exec_p) + new_cost["total"]
                    if self.cash >= new_req:
                        self.cash -= new_req
                        self.positions[sym] = self.positions.get(sym, 0.0) + new_vol
                        self._record_trade(t_date, sym, OrderDirection.LONG, new_vol, exec_p, new_cost)
